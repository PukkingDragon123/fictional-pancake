...
const Main = (() => {
  ...
  let dig = 1, down = false, lastP = null, downP = null, moved = 0, panning = false, screenP = { x: 320, y: 240 };
  ...
  function stroke(p) {
    if (!lastP) { Grove.press(p.x, p.y, true, dig); lastP = p; return; }
    ...
    for (let i = 1; i <= n; i++) Grove.press(lastP.x + (dx * i) / n, lastP.y + (dy * i) / n, false, dig);
    if (n > 0) lastP = p;
  }
  ...
  function bind() {
    canvas.addEventListener('pointerdown', (e) => {
      ...
      const p = pm;
      if (e.button === 2) {
        // with the shovel out, the right button digs down instead of opening the tray
        if (G.mode === 'grove' && (TOOL_BY_KEY[G.tool] || {}).terra && !UI.wheelOpen()) {
          down = true; downP = pm; moved = 0; panning = false; screenP = pm; dig = -1;
          lastP = world(pm);
          Grove.press(lastP.x, lastP.y, true, dig);
          return;
        }
      ...
      down = true; lastP = null; downP = p; moved = 0; panning = false; dig = e.shiftKey ? -1 : 1;
      screenP = p;
      if (G.mode === 'intro') { Intro.press(p.x, p.y); return; }
      if (G.mode === 'grove') {
        const w = world(p);
        if (Guide.hit(w.x, w.y)) { Guide.poke(); down = false; return; }
        const consumed = Grove.press(w.x, w.y, true, dig);
  ...
  function frame(ts) {
    ...
    if (playing && !G.paused && down && !panning && G.mode === 'grove' && (TOOL_BY_KEY[G.tool] || {}).terra && lastP) {
      Grove.press(lastP.x, lastP.y, false, dig);
    }
    FX.updateWorld(G.paused ? 0 : real);
  ...
  function splash() {
    g.fillStyle = '#f3dfb4'; g.fillRect(0, 0, W, H);
    Art.vband(g, 0, 0, W, H, '#bfe3f2', '#f6e3b8', 8);
    FX.pixelText(g, 'WOMBAT FARM', W / 2, H / 2 - 22, { color: '#ffd45e', size: 26, ink: 3, inkColor: 'rgba(58,36,20,0.8)' });
    FX.pixelText(g, 'opening the gate', W / 2, H / 2 + 14, { color: '#7a5634', size: 14, ink: false });
  }
  async function init() {

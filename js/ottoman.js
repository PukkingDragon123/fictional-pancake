...
const Ottoman = (() => {
  ...
  function enter() {
    layout();
    scroll = tscroll = 0; hover = null; t = 0; fade = 1; fadeTo = null; menu.open = false;
  ...
  function press(x, y) { if (menu.open) { menuPress(x, y); return; } drag = { x, s: tscroll }; moved = 0; }
  function move(x, y) {
    if (menu.open) { menuMove(x, y); return; }
    if (!drag) return;
    moved = Math.max(moved, Math.abs(x - drag.x));
    tscroll = U.clamp(drag.s - (x - drag.x), 0, worldW - VW); scroll = tscroll;
  }
  ...
  function wheel(dy) {
    ...
    tscroll = U.clamp(tscroll + dy, 0, worldW - VW);
  }
  ...
  function render(g) {
    renderShop(g);
    if (menu.open) drawMenu(g);
    if (fade > 0) { g.fillStyle = `rgba(20,8,4,${fade.toFixed(2)})`; g.fillRect(0, 0, VW, VH); }
  ...
  return {
    ...
    layout, openMenu, closeMenu, get menuOpen() { return menu.open; },

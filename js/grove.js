...
const Grove = (() => {
  ...
  for (let d = 0; d < 5; d++) {
    ...
    for (let i = 0; i < n; i++) {
      row.push({
        ...
        y: SKY - 40 + d * 13 + R0() * 6,
        kind: ['gnarl', 'pine', 'oak', 'oak', 'birch'][Math.floor(R0() * 5)],
        v: Math.floor(R0() * 4),
  ...
  // ---- the air ---------------------------------------------------------------
  // Pollen drifting in the sun, and fireflies once it gets dark. Nothing in it
  // means anything; it just makes the place feel alive.
  const MOTE = [];
  function seedMotes() {
    MOTE.length = 0;
    const r = Art.rng(3141);
    for (let i = 0; i < 160; i++) {
      MOTE.push({ x: r() * W, y: GROUND + r() * (H - GROUND), ph: r() * TAU, rise: 3 + r() * 8, v: Math.floor(r() * 3) });
    }
  }
  function drawMagic(g, L, R) {
    ...
    for (const m of MOTE) {
      ...
      if (night > 0.35) {
        // fireflies: a green-gold dot that swells and fades
        const a = U.clamp((night - 0.35) * 1.6 * k, 0, 0.95);
        if (a < 0.05) continue;
        Art.rect(g, px - 1, py - 1, 4, 4, U.rgba('#d8ff70', a * 0.25));
        Art.rect(g, px, py, 2, 2, U.rgba('#fff4a0', a));
      } else if (m.v === 0) {
        // pollen, only the odd one, catching the light
        const a = 0.25 + k * 0.35;
        Art.rect(g, px, py, 1, 1, U.rgba(k > 0.7 ? '#fff6d8' : '#f4dc8a', a));
      }
    }
  }
  ...
  function press(x, y, first, dir) {
    ...
    // ---- the shovel -------------------------------------------------------
    // A brush like the hoe and the can: a footprint at the size you set, held
    // and dragged, working the whole time the button is down. The left button
    // heaps the ground up, the right one (or Shift) digs it out.
  ...
  // ---- scene --------------------------------------------------------------
  function render(g) {
    // how far the grass has come back; the mood never drops below a sunny
    // afternoon however bare the ground still is
    const f = Math.max(0.75, World.fraction());
    const cam = FX.cam;
    ...
    const top = U.mix(U.mix('#16244a', '#6fb4e2', day), '#9aa8b4', cov * 0.7);
    const bot = U.mix(U.mix('#34406a', '#d8f0e2', day), '#c4ccd0', cov * 0.7);
    for (let i = 0; i < 12; i++) {
    ...
    for (let d = 0; d < layers.length; d++) {
      ...
      for (const m of mist) {
        ...
        for (let i = 0; i < 7; i++) {         // fog bank, banded from the middle out
          ...
          Art.dither(g, mx, m.y - 14 + i * 4, m.w, 4, '#f4f2ea', aa * 0.6);
        }
      }
    }
    ...
    for (const m of mist) {
      ...
      for (let i = 0; i < 6; i++) Art.dither(g, mx, GROUND - 4 + i * 6, m.w * 1.4, 6, '#f6f2e6', a * 0.5 * (1 - i / 6));
    }

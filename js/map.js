...
const Atlas = (() => {
  ...
  const HW = 320, HH = 180;
  // The district on a summer afternoon, drawn from above: soft greens, blue
  // water, sandy lanes and red roofs.
  const G0 = '#6fae48', G1 = '#7cba50', G2 = '#88c65a', G3 = '#9ad06a';   // grass
  const GD = '#5e9e3e', GD2 = '#4e8a32';                                  // grass shade
  const W0 = '#2f7aa8', W1 = '#4a9ac4', W2 = '#6ab8dc', W3 = '#a8e0f0';   // water
  const SAND = '#dcc48c', SAND2 = '#bca46c';
  const RD = '#dcbc80', RDE = '#a8845a';                                  // paths
  const OUT = '#1e2a14';                                                  // the one dark line
  const TR0 = '#244a1c', TR1 = '#3a6a2a', TR2 = '#4f8a38', TR3 = '#6aaa4a';
  const BLOS = '#f0a0b8', BLOS2 = '#ffd0dc';                              // cherry blossom
  const PIN0 = '#1e3a1a', PIN1 = '#2e5a28', PIN2 = '#4a7a3a';
  const RF = ['#c8584a', '#d88a4a', '#6a9ac8', '#a8584a', '#e0b04a'];     // roof colours
  const WALL = '#ecdcbc', WALL2 = '#c8b490';
  ...
  function cuteTree(q, x, y, s, kind, seed) {
    ...
    Art.ell(q, x, y - s * 1.25, s * 1.15, s * 1.05, kind === 'blossom' ? '#a85a74' : TR0);
    Art.ell(q, x, y - s * 1.3, s * 1.0, s * 0.9, kind === 'blossom' ? '#d8809a' : TR1);
  ...
  function hoverAt(x, y) {
    ...
    if (!siteOpen(hover)) return `<b>?</b><br>${hover.gate && !hover.gate(G) ? hover.why : 'not yet'}`;
  ...
  const siteOpen = (s) => !!s && (!s.gate || s.gate(G));
  function update(dt) {
    Drive.update(dt);
  }
  ...
  function render(g) {
    ...
    // ---- birds going over, lazily -------------------------------------------
    for (const b of bats) {
      const x = ((b.x + t * b.sp) % (VW + 40)) - 20;
      const y = b.y + Math.sin(t * 0.8 + b.ph) * b.amp * 0.3;
      const flap = Math.sin(t * 6 + b.ph) * 2;
      g.fillStyle = 'rgba(40,34,30,0.7)';
    ...
    for (const d of drift) {                     // spores riding the cold air
      ...
      g.fillStyle = `rgba(255,244,200,${(0.12 + 0.16 * Math.sin(t * 2 + d.ph)).toFixed(2)})`;
      g.fillRect(Math.round(x), Math.round((y + VH) % VH), 1, 1);
    }
    // ---- the grade: a warm summer afternoon over the whole district ---------
    g.save();
    g.globalCompositeOperation = 'soft-light';
    g.fillStyle = '#ffe0a0'; g.globalAlpha = 0.4; g.fillRect(0, 0, VW, VH);
    g.restore();
    g.save();
    ...
    Art.vignette(g, VW, VH, '#3a2a14', 0.3, 2.4, 0.4);

    // title banner
    banner(g, 'AROUND THE FARM', 320, 24);

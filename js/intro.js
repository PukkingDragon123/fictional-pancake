...
const Intro = (() => {
  ...
  const CLIPS = [
    ...
    { job: true, cap: 'HIRING: WOMBAT CARETAKER', tag: '@wombatfarm.tas', likes: 12, kind: 'job' },
  ];
  // the conversation, one bubble at a time
  const CHAT = [
    { who: 'her', s: 'you saw the post' },
    { who: 'her', s: 'lovely! most people scroll past' },
    { who: 'you', s: 'is the job real' },
    { who: 'her', s: 'very real, dear. it is the farm next to mine' },
    { who: 'her', s: 'a garden gone wild and wombats who need a friend' },
    { who: 'you', s: 'what is the pay' },
    { who: 'her', s: 'W$300 to start, and all the veg you can grow' },
    { who: 'her', s: 'come on down. the kettle will be on' },
    { who: 'link', s: 'wombatfarm.tas/caretaker' },
  ];
  ...
  function update(dt) {
    ...
    if (phase === 'drive' || phase === 'arrive') driveTick(dt);
  ...
  function bedroom(g) {
    g.fillStyle = '#3a2820'; g.fillRect(0, 0, VW, VH);
    g.fillStyle = '#4e3628'; g.fillRect(0, 0, VW, 258);
    for (let x = 0; x < VW; x += 26) { g.fillStyle = 'rgba(255,255,255,0.025)'; g.fillRect(x, 0, 12, 258); }
    g.fillStyle = '#2e2018'; g.fillRect(0, 252, VW, 8);
    // ---- left: the window, a gum outside, and a shelf of toys -------------
    g.fillStyle = '#2a1a12'; g.fillRect(14, 16, 150, 116);
    ...
    g.fillStyle = '#1e140e'; g.fillRect(10, 12, 158, 5); g.fillRect(10, 128, 158, 7); g.fillRect(86, 20, 5, 108);
    g.globalAlpha = 0.1; g.fillStyle = '#9fb6ff'; Art.poly(g, [[18, 128], [160, 128], [200, 252], [0, 252]], '#9fb6ff'); g.globalAlpha = 1;
    g.fillStyle = '#6a4a30'; g.fillRect(8, 176, 176, 9);
    g.fillStyle = '#8a6440'; g.fillRect(8, 176, 176, 3);
    g.fillStyle = '#4a3222'; g.fillRect(14, 185, 7, 7); g.fillRect(172, 185, 7, 7);
    ...
    poster(g, 440, 22, 116, 96, '#d8c49a', '#6a4a30', (x, y, w, h) => {
    ...
    poster(g, 440, 132, 78, 62, '#5a9a4a', '#2f5a2a', (x, y, w, h) => {
      for (let i = 0; i < 24; i++) { g.fillStyle = 'rgba(255,255,255,0.5)'; g.fillRect(x + (i * 29) % w, y + (i * 17) % h, 1, 1); }
      Sprites.blit(g, x + w / 2, y + h - 8, 'happy', Math.floor(t * 3), 'sand', 1, 'adult', 1.1);
      Font.draw(g, 'GROW VEG', x + w / 2, y + 6, { scale: 1, color: '#fff4d8', align: 'center' });
    });
    g.fillStyle = '#4a3222'; g.fillRect(430, 206, 210, 11);
    g.fillStyle = '#6a4a30'; g.fillRect(430, 206, 210, 4);
    g.fillStyle = '#4a3222'; g.fillRect(446, 217, 9, 42); g.fillRect(614, 217, 9, 42);
    ...
    g.fillStyle = '#6a4a30'; g.fillRect(586, 162, 7, 44);
    ...
    for (let i = 0; i < 22; i++) {
      ...
      g.fillStyle = '#4a3a2a'; g.fillRect(lx, ly, 29, 1);
      ...
      g.fillStyle = on ? col : '#6a5a4a'; g.fillRect(lx + 12, ly + 1, 3, 5);
    }
    // ---- the bed you are lying on -----------------------------------------
    g.fillStyle = '#8a4a3e'; g.fillRect(0, 258, VW, VH - 258);
    g.fillStyle = '#a85a4a'; g.fillRect(0, 258, VW, 12);
    for (let i = 0; i < 6; i++) { g.fillStyle = '#9a5244'; g.fillRect(-20 + i * 128, 278, 108, 11); }
    g.fillStyle = '#c8805a'; g.fillRect(0, 300, 210, 60);
    for (let i = 0; i < 5; i++) { g.fillStyle = '#e0a070'; g.fillRect(8 + i * 40, 306 + (i % 2) * 9, 32, 6); }
    g.fillStyle = '#c8805a'; g.fillRect(440, 296, 200, 64);
    for (let i = 0; i < 4; i++) { g.fillStyle = '#e0a070'; g.fillRect(452 + i * 46, 304 + (i % 2) * 9, 36, 6); }
    ...
    g.fillStyle = 'rgba(30,16,10,0.22)'; g.fillRect(0, 0, VW, VH);
  }
  ...
  function clipArt(g, kind, x, y, w, h, tt) {
    ...
    if (kind === 'job') {
      ...
      Font.draw(g, 'WOMBAT FARM, TAS', x + w / 2, y + 8, { scale: 1, color: '#ffe497', align: 'center' });
    } else if (kind === 'graze') {
  ...
  function chat(g) {
    ...
    Font.draw(g, 'AUNT FERN', SX + 27, SY + 24, { scale: 1, color: '#e6e6f0', align: 'left' });
    ...
    for (let i = 0; i < shown; i++) {
      ...
      if (m.who === 'link') {
        ...
        Font.draw(g, 'wombatfarm.tas', SX + 16, y + 4, { scale: 1, color: '#7fc6e0', align: 'left' });
  ...
  function web(g) {
    ...
    Font.draw(g, 'wombatfarm.tas', SX + 15, SY + 23, { scale: 1, color: '#b9b6c6', align: 'left' });
    ...
    Font.draw(g, 'NO EXPERIENCE NEEDED', SX + SW / 2, y + 20, { scale: 1, color: '#cfccdc', align: 'center' });
    Font.draw(g, 'JUST A KIND HEART', SX + SW / 2, y + 31, { scale: 1, color: '#cfccdc', align: 'center' });
    Font.draw(g, 'ONE (1) FARM INCLUDED', SX + SW / 2, y + 42, { scale: 1, color: '#cfccdc', align: 'center' });
    y += 70;
    // the small print that scrolls past
    g.save(); g.beginPath(); g.rect(SX + 6, y, SW - 12, 14); g.clip();
    Font.draw(g, 'THE WOMBATS COME WITH THE FARM. THEY ARE VERY SOFT. BRING A HAT AND A GOOD PAIR OF BOOTS.   ', SX + 6 + ((-webT * 30) % 700), y + 3, { scale: 1, color: '#6a6880', align: 'left' });
    g.restore();
    y += 20;
    ...
    Font.draw(g, 'AND TO PETTING THEM DAILY', SX + SW / 2, acceptR.y + 42, { scale: 1, color: '#7a7490', align: 'center' });
    y = acceptR.y + 56;
    ...
    for (const [who, what] of [['b.wombat44', '"i have 9 wombats now"'], ['grove_fan', '"the soil is amazing"'], ['t. keeper', '"best decision ever!!"']]) {
    ...
    Font.draw(g, 'C 1998 WOMBAT FARM TAS', SX + SW / 2, y + 4, { scale: 1, color: '#4a4860', align: 'center' });
  ...
  // ---- the drive: the same road every journey uses -------------------------
  // The opening haul is the travel scene with the dash swapped for a title.
  const driveState = { t: 0, dur: 5.6, to: 'WOMBAT FARM', km: 5, cars: [], dust: [],
    title: 'TASMANIA', sub: 'two hours from the airport', fade: false };
  function drive(g) {
    driveState.t = t;
    Drive.paint(g, driveState);
  }
  // the oncoming traffic is state, so it needs a tick of its own here
  function driveTick(dt) {
    const st = driveState;
    if (st.cars.length < 2 && Math.random() < dt * 0.5) st.cars.push({ x: 760, sp: 260 + Math.random() * 180, lane: Math.random() < 0.5 ? 0 : 1 });
    for (let i = st.cars.length - 1; i >= 0; i--) { st.cars[i].x -= st.cars[i].sp * dt; if (st.cars[i].x < -180) st.cars.splice(i, 1); }
  }
